...
class AsignaturaTestCase ( unittest.TestCase ) :
    def setUp ( self ) :
        ...
        self.sorteo = ControladorEstudiante ()
    ...
    def test_agregar_estudiante ( self ) :
        resultado = self.sorteo.agregar_estudiante ( apellidoPaterno = "Cardenas" , apellidoMaterno = "Segura" , nombres = "Karoline" ,
                                   elegible = True )
        self.assertEqual ( resultado , True )

    def test_agregar_estudiante_repetido(self):
        resultado = self.sorteo.agregar_estudiante(apellidoPaterno = "Ramos" , apellidoMaterno = "Ortega" , nombres = "Juan Carlos" ,
                                   elegible = True)
        self.assertNotEqual(resultado, True)

    def test_verificar_almacenamiento_agregar_estudiante(self):
        self.sorteo.agregar_estudiante(apellidoPaterno = "Cardenas" , apellidoMaterno = "Segura" , nombres = "Karoline" ,
                                   elegible = True)

        self.session = Session()
        estudiante = self.session.query(Estudiante).filter(Estudiante.apellidoPaterno == "Cardenas",Estudiante.apellidoMaterno == "Segura", Estudiante.nombres=="Karoline",Estudiante.elegible==True).first()
        self.assertEqual("Cardenas", estudiante.apellidoPaterno)
        self.assertEqual("Segura", estudiante.apellidoMaterno)
        self.assertEqual("Karoline", estudiante.nombres)
